...
**Probe execution throughput.** One leader's worker pool today. `--probe-concurrency` caps parallel HTTP/DNS executions. Horizontal scaling of probe workload is **Phase 14** (independent `synthetics-operator-prober` deployment pulling from a NATS work queue).
