...
| `synthetics_operator_nats_publish_failed_total` | counter | NATS publish failures per component (sidecar, prober) |
...
```
controller          → NATS work queue  ← probers (compete for jobs)
...
**Probe execution throughput.** The `synthetics-operator-prober` Deployment pulls jobs from a NATS queue group, so throughput scales with `prober.replicaCount` — or with KEDA, via `prober.autoscaling`. Each `ProbeJob` carries the full spec it needs to execute, so a worker holds no state and a replica that started a second ago can run the first job it wins. That is what makes scaling safe: with jobs that carried only `{kind, namespace, name}`, a worker had to have already seen a spec publish, and specs are only published on change — so every scaled-up replica silently discarded its share of runs until the next spec edit.

**NATS.** Two supported postures, selected by Helm values:
- **Bundled** (`nats.enabled: true`): a `StatefulSet`, no JetStream. `nats.replicas: 3` starts a full-mesh route cluster (`--cluster` on `:6222`, peers addressed through a headless Service), so losing one server doesn't stop delivery and a node drain is survivable. There's still no persistence — messages in flight when a server dies are lost. Fine for dev and small production setups.
...
# synthetics-operator-prober (per replica)
resources:
  requests:
    cpu: 50m
    memory: 64Mi
...
**`synthetics-operator-prober` ClusterRole:**
...
- `synthetics-operator-prober`: `:8081` from kubelet only
...
- `synthetics-operator-prober` deployment: stateless workers consume jobs from NATS work queue, publish results to NATS result stream
