...
func (r *PlaywrightTestReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &test); err != nil {
		if apierrors.IsNotFound(err) {
			return ctrl.Result{}, r.Publisher.PublishSpec(ctx, tombstone(results.KindPlaywrightTest, req.Namespace, req.Name))
		}
		return ctrl.Result{}, err
	}

	if !test.DeletionTimestamp.IsZero() {
		return ctrl.Result{}, r.Publisher.PublishSpec(ctx, tombstone(results.KindPlaywrightTest, test.Namespace, test.Name))
	}

	if err := r.Publisher.PublishSpec(ctx, playwrightTestSpecUpdate(&test)); err != nil {
		return ctrl.Result{}, err
	}
